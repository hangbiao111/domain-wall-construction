...
def set_dw_center_file(n):
    ...
    for line in all_lines:
        if all_lines.index(line) >= 8:
            ...
            num2_f = float(num2)
            num3_f = float(num3)
            if 0.25 / float(n) < num1_f < 0.75 / float(n) or 0.5 + 0.25 / float(n) < num1_f < 0.5 + 0.75 / float(
                    n):
                if 0.3 <= num3_f <= 0.7:
                    num3_f = float(0.5)
                    str_list_new.append('{:.16f}'.format(num2_f))
                    ...
                    str_list_new.append('\n')
                    new_line += "   ".join(str(i) for i in str_list_new)
            elif num1_f <= 0.75 / float(n) or 0.5 + 0.75 / float(n) <= num1_f <= 1:
                if num3_f != 0:
                    num3_f = 1.0 - num3_f
                    str_list_new.append('{:.16f}'.format(num2_f))
                    str_list_new.append('{:.16f}'.format(float(num3_f)))
...
def set_bloch_file(n):
    ...
    for line in all_lines:
        if all_lines.index(line) == 3:
            ...
            num2_f = float(num2)
            num3_f = float(num3)
            if 0.25 / float(n) < num1_f < 0.75 / float(n) and num2_f == 0.5 and num3_f == 0.5:
                num2_f = float(num2) - 0.16 / lat_y_f
            elif 0.5 + 0.25 / float(n) < num1_f < 0.5 + 0.75 / float(n) and num2_f == 0.5 and num3_f == 0.5:
                num2_f = float(num2) + 0.16 / lat_y_f
            else:
                num2_f = float(num2)
            str_list_new.append('{:.16f}'.format(num1_f))
